...
def _test_rnn_rand_vec(x,y):
    ...
    ff = f1_score(train_labels, train_pred, average='macro')       #
        # print("loss:{}, i:{} ".format(loss.data,i))
    print(ff)
    # h = []
    # for j in range(x_test.size()[0]):
    #     encoder_hidden = model.init_hidden()
    #
    #     input_data2 = torch.autograd.Variable(x_test[j])
    #     output_labels2 = torch.autograd.Variable(torch.LongTensor([y_test[j]]))
    #
    #     encoder_outputs2,encoder_hidden2 = model(input_data2,encoder_hidden)
    #
    #     encoder_outputs2 = torch.argmax(encoder_outputs2.data)
    #
    #     # encoder_outputs2 = encoder_outputs2.numpy()
    #     # h.append(encoder_outputs2)
    #     output_labels2 = output_labels2.numpy()
    #     # print(output_labels2)
    #     h.append(output_labels2)
        # print(encoder_outputs2, output_labels2)
        # print('--' * 20)
    # print(h)
    print('--'*20)
    # ff = f1_score(output_labels2, encoder_outputs2, average='macro')
    # print(ff)
    return


h = PreProcessor('./data/validationset.csv')
s,c = h.process()
x = _test_rnn_rand_vec(s,c)
